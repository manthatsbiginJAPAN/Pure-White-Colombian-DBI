...
namespace FrontEndV0._1.forms
{
      ...
      * private void populateSupervisors()
        ...
        private void cbUnitID_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i <= rowcnt - 1; i++)
            {
                ...
                if (tms.Tables[0].Rows[i][0].ToString() == cbUnitID.SelectedItem.ToString())
                {
                    sem = tms.Tables["teamcursor"].Rows[i][1].ToString();
        ...
        private void cbSemester_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i <= rowcnt - 1; i++)
            {
                ...
                if (tms.Tables[0].Rows[i][0].ToString() == cbUnitID.SelectedItem.ToString() && tms.Tables[0].Rows[i][1].ToString() == cbSemester.SelectedItem.ToString())
                {
                    year = tms.Tables["teamcursor"].Rows[i][2].ToString();
 ...
 //FIX               if (tms.Tables[0].Rows[i][0].ToString() == cbUnitID.SelectedItem.ToString() && tms.Tables[0].Rows[i][1].ToString() == cbSemester.SelectedItem.ToString() && tms.Tables[0].Rows[i][2].ToString() == cbYear.SelectedItem.ToString())
                {
                    team = tms.Tables["teamcursor"].Rows[i][3].ToString();
